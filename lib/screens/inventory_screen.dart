...
class _InventoryScreenState extends State<InventoryScreen> {
  ...
  Future<void> _fetchProducts() async {
    try {
      final productsResult = await ApiService.getAdminProducts();
      final productsData = productsResult['data'];
      final productsContent = (productsData is Map && productsData['content'] is List)
          ? productsData['content'] as List
          : (productsData is List ? productsData : const []);

      final summaryResult = await ApiService.getAdminInventorySummary()
          .catchError((_) => <String, dynamic>{});
      final summaryData = (summaryResult['data'] is Map)
          ? (summaryResult['data'] as Map<String, dynamic>)
          : <String, dynamic>{};
      final summaryContent = summaryData['content'] is List
          ? summaryData['content'] as List
          : const [];

      final summaryById = <String, Map<String, dynamic>>{};
      for (final item in summaryContent) {
        if (item is Map) {
        ...
        _products = productsContent
            ...
            .map<Product>(
              (p) => Product(
                ...
                quantity: _toInt(
                  summaryById[p['id']?.toString()]?['totalStock'] ??
                      p['currentStock'] ??
                      p['quantity'],
                ),
                minStock: _toInt(
                  summaryById[p['id']?.toString()]?['minStock'] ??
                      p['minStock'],
                ),
                maxStock: _toInt(
                  summaryById[p['id']?.toString()]?['maxStock'] ??
                      p['maxStock'],
                ),
                locationLabel:
                    p['locationLabel']?.toString() ??
                    p['location']?.toString() ??
                    '—',
                price: _toDouble(
                  summaryById[p['id']?.toString()]?['price'] ?? p['price'],
                ),
              ),
            )
            .toList();
  ...
  Widget _buildExpandedDetail(Product p) {
    final stockPercent = p.maxStock > 0
        ? (p.quantity / p.maxStock).clamp(0.0, 1.0)
        : 0.0;
      ...
      child: Column(
        ...
        children: [
          Wrap(
            ...
            children: [
              SizedBox(
                width: isCompact ? null : null,
                child: _detailItem('Price', '${p.price.toStringAsFixed(0)} DA'),
              ),
              SizedBox(
                width: isCompact ? null : null,
                child: _detailItem('Min Stock', '${p.minStock}'),
              ),
              SizedBox(
                width: isCompact ? null : null,
                child: _detailItem('Max Stock', '${p.maxStock}'),
              ),
              SizedBox(
                width: isCompact ? null : null,
                child: _detailItem('Last Updated', _fmtDate(p.lastUpdated)),
              ),
            ],
          ),
          const SizedBox(height: 14),
          ...
          Container(
            ...
            child: Row(
              children: [
                ...
                const Expanded(
                  child: Text(
                    'AI Forecast: Demand expected to increase 15% next month',
                    style: TextStyle(fontSize: 11, color: AppColors.aiBlue),
                  ),
                ),
                Container(
                  padding: const EdgeInsets.symmetric(
  ...
  void _showProductDialog({Product? existing}) {
    ...
    final locationCtrl = TextEditingController(
      text: existing?.locationLabel ?? 'A-01',
    );
    ...
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        title: Row(
          children: [
            Icon(
              existing == null ? Icons.inventory_2_outlined : Icons.edit,
        ...
        actions: [
          ...
          FilledButton(
            onPressed: () async {
              ...
              try {
                if (existing == null) {
                  // POST /admin/products — ProductCreateRequest
                  ...
                  await ApiService.updateProduct(existing.id!, {
                    'name': name,
                    'category': category,
                    'price': price,
                    'minStock': minStock,
        ...
        actions: [
          ...
          FilledButton(
            ...
            onPressed: () {
              ...
              // POST /admin/inventory/adjustment — StockAdjustmentRequest
              final adjustQty = q - p.quantity;
              ApiService.adjustStock({
                    'productId': p.id,
                    'locationId':
                        '00000000-0000-0000-0000-000000000000', // placeholder
                    'adjustmentQuantity': adjustQty,
                    'reason': 'Manual stock adjustment from UI',
                  })
                  .then((_) {
                    _showSnack(
                      'Stock adjusted (${adjustQty >= 0 ? "+$adjustQty" : "$adjustQty"})',
                    );
                    _fetchProducts();
                  })
                  .catchError((e) {
